...
function checkoutReducer(state: CheckoutState, action: CheckoutAction): CheckoutState {
  switch (action.type) {
    ...
    case 'UPDATE_CUSTOMER_INFO': {
      ...
      const updatedData = updateCheckoutData(state, { customerInfo: updatedCustomerInfo });
    ...
    case 'UPDATE_BILLING_INFO': {
      ...
      const updatedData = updateCheckoutData(state, { billingInfo: updatedBillingInfo });
    ...
    case 'UPDATE_PAYMENT_INFO': {
      ...
      const updatedData = updateCheckoutData(state, { paymentInfo: updatedPaymentInfo });
    ...
    case 'SELECT_SHIPPING_OPTION': {
      const updatedData = updateCheckoutData(state, { selectedShippingOption: action.payload });
    ...
    case 'APPLY_DISCOUNT': {
      const updatedData = updateCheckoutData(state, { appliedDiscount: action.payload });
    ...
    case 'REMOVE_DISCOUNT': {
      const updatedData = updateCheckoutData(state, { appliedDiscount: undefined });
    ...
    case 'VALIDATE_FORM': {
      ...
      return {
        ...state,
        errors: deduped,
        isValid: Object.values(deduped).every(sectionErrors => sectionErrors.length === 0)
      };
    }
...
export function CheckoutProvider({ 
  ...
  const canProceedToStep = useCallback((step: CheckoutStep) => {
  ...
  // Form updates
  const updateCustomerInfo = useCallback((info: Partial<CheckoutCustomerInfo>) => {
    dispatch({ type: 'UPDATE_CUSTOMER_INFO', payload: info });
  }, []);

  const updateShippingInfo = useCallback((info: Partial<CheckoutShippingInfo>) => {
    dispatch({ type: 'UPDATE_SHIPPING_INFO', payload: info });
  }, []);

  const updateBillingInfo = useCallback((info: Partial<CheckoutBillingInfo>) => {
    dispatch({ type: 'UPDATE_BILLING_INFO', payload: info });
  }, []);

  const updatePaymentInfo = useCallback((info: Partial<CheckoutPaymentInfo>) => {
  ...
  // Shipping
  const loadShippingOptions = useCallback(async (zipCode: string) => {
    ...
    try {
      ...
      const hasSubscription = state.items.some(item => item.type === 'subscription');
      if (hasSubscription) {
  ...
  const clearErrors = useCallback((section?: keyof CheckoutValidationErrors) => {
  ...
  const processOrder = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      dispatch({
        type: 'SET_ERRORS',
        payload: { 
          general: [error instanceof Error ? error.message : 'Order processing failed'] 
        }
      });
  ...
  const isStepCompleted = useCallback((step: CheckoutStep): boolean => {

...
export default function OrderReviewStep({ onPlaceOrder, onBack }: OrderReviewStepProps) {
  const { 
    items,
    customerInfo,
    shippingInfo,
      ...
      >
              ...
              <div key={item.id} className="flex items-center space-x-4">
                <div className="flex-shrink-0 w-16 h-16 relative bg-gray-100 rounded-lg overflow-hidden">
                  {item.product.images && item.product.images[0] && (
                    <Image
                      src={`${(() => { const u = item.product.images[0].url || ''; if (!u) return '/images/placeholder.png'; return u.startsWith('http') || u.startsWith('data:') ? u : (u.startsWith('/') ? u : '/' + u); })()}`}
                ...
                <div className="flex-1">
                  ...
                  {item.packageConfiguration && (
                    <p className="text-xs text-gray-500">
                      {item.packageConfiguration.planType} plan - {item.packageConfiguration.totalPacks} packs
                    </p>
                  )}
                  <p className="text-xs text-gray-500">Deliveries Mondays & Thursdays</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-gray-600">Qty: {item.quantity}</p>
                  <p className="font-medium text-gray-900">${item.totalPrice.toFixed(2)}</p>
                  {item.discountAmount && item.discountAmount > 0 && (
                    <p className="text-xs text-green-600">-${item.discountAmount.toFixed(2)} savings</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {customerInfo.marketingOptIn && (
                <p className="text-green-600 text-xs">✓ Subscribed to marketing updates</p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              {shippingInfo.deliveryInstructions && (
                <p className="text-gray-600">
                  <span className="font-medium">Instructions:</span> {shippingInfo.deliveryInstructions}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-2 text-sm">
              {paymentInfo.methodType === 'credit_card' || paymentInfo.methodType === 'debit_card' ? (
                <>
                  <p><span className="font-medium">Card:</span> {formatCardNumber(paymentInfo.cardNumber || '')}</p>
                  <p><span className="font-medium">Name:</span> {paymentInfo.cardholderName}</p>
                  <p><span className="font-medium">Expires:</span> {paymentInfo.expiryMonth}/{paymentInfo.expiryYear?.slice(-2)}</p>
                </>
              ) : (
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                {selectedShippingOption.carrierName && (
                  <p className="text-xs text-gray-500">via {selectedShippingOption.carrierName}</p>
                )}
              </div>
              <div className="text-right">
                <p className="font-medium">
                  {selectedShippingOption.price === 0 ? 'Free' : `$${selectedShippingOption.price.toFixed(2)}`}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          {appliedDiscount ? (
            <div className="flex items-center justify-between">
              ...
              <p className="font-medium text-green-800">-${appliedDiscount.appliedAmount?.toFixed(2)}</p>
            </div>
          ) : (
            <p className="text-sm text-gray-600">Add a discount code at this step.</p>
          )}
          {errors.general.length > 0 && (
            <ValidationMessage message={errors.general.join(' • ')} type="error" className="mt-3" />
          )}
        </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-yellow-700 mt-1">
                ...
                Your order will be processed immediately and cannot be cancelled once confirmed.
              </p>
            </div>
          </div>
        </div>

...
export function VarietySelector({
  ...
  const [imageIndexByVariety, setImageIndexByVariety] = useState<
  ...
  const totalSelected = varietySelections.reduce(
  ...
  const getVarietyTheme = (varietyId: string) => {
    const variety =
      MICROGREENS_VARIETIES[varietyId as keyof typeof MICROGREENS_VARIETIES];
  ...
  const handleQuantityChange = (
    ...
    const currentSelection = varietySelections.find(
      s => s.varietyId === varietyId
    );
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm">
      ...
      <p className="text-gray-600 mb-6">
        {planType === 'starter'
          ? 'Select any number of packs.'
          : `Select exactly ${maxPacks} packs.`}
      </p>

      <div className="space-y-6">
        {Object.entries(MICROGREENS_VARIETIES).map(([varietyId, variety]) => {
          const selection = varietySelections.find(
            s => s.varietyId === varietyId
          );
          ...
          return (
            <div
              key={varietyId}
              className={`border rounded-xl p-5 ${quantity > 0 ? 'ring-1 ring-fuelfoods-green-500' : 'border-gray-200'}`}
            >
                ...
                <div className="mb-4 lg:mb-0 lg:mr-5">
                  <div className="w-28 md:w-32 mx-auto">
                    <div className="w-full h-28 md:h-32 rounded-xl border border-gray-200 bg-white overflow-hidden flex items-center justify-center">
                      <img
                        src={
                          (VARIETY_IMAGES[varietyId] || [variety.image])[
                            imageIndexByVariety[varietyId] || 0
                          ]
                        }
                        ...
                        onClick={() =>
                          setLightbox({
                            open: true,
                            src: (VARIETY_IMAGES[varietyId] || [variety.image])[
                              imageIndexByVariety[varietyId] || 0
                            ],
                          })
                        }

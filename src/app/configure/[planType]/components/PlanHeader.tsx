  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900">{planConfig.name}</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          {planType !== 'starter' && (
            <div className="w-40 h-2 rounded-full bg-gray-200 overflow-hidden">
              <div className="h-2 bg-fuelfoods-green-500" style={{ width: `${Math.min((totalSelected / planConfig.packsRequired) * 100, 100)}%` }} />
            </div>
          )}
        </div>
      </div>

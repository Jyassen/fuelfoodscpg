...
export function createEliteSubscriptionWithDiscountExample(): CheckoutData {
  ...
  const appliedDiscount = {
    ...
    isValid: true,
    appliedAmount: Math.round(eliteItem.totalPrice * 0.20 * 100) / 100,
 ...
 */
export function createOrderSummaryExample(checkoutData: CheckoutData): OrderSummary {
  ...
  const orderSummary: OrderSummary = {
    items: checkoutData.items.map(item => ({
      ...
      totalPrice: item.totalPrice,
      type: item.type,
      subscriptionDetails: item.type === 'subscription' ? {
